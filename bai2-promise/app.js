...
function sendRequest(method, url, retry = true) {
    ...
    xhr.onload = function () {
      if (xhr.status >= 200 && xhr.status < 400) {
        ...
        if (retry) {
          // Retry sau 2s
          setTimeout(() => {
            sendRequest(method, url, false).then(resolve).catch(reject);
          }, 2000);
        } else {
          reject(xhr);
        }
  ...
  sendRequest("GET", "https://jsonplaceholder.typicode.com/users")
    .then((users) => {
      if (users) {
        ...
        return sendRequest("GET", `https://jsonplaceholder.typicode.com/posts?_limit=${POSTS_LIMIT}&_page=${postPage}`);
      }
    })
    .then((posts) => {
      renderPost(posts);
...
HTML["#load-more-posts-btn"].addEventListener("click", (e) => {
  ...
  sendRequest("GET", `https://jsonplaceholder.typicode.com/posts?_limit=${POSTS_LIMIT}&_page=${++postPage}`)
    .then((posts) => {
      if (posts.length) {
        renderPost(posts);
      } else {
